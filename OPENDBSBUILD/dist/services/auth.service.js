...
class AuthService {
    ...
    authenticateWithApiKey(apiKey) {
        ...
        const user = users.find((u) => u.apiKey && (0, security_middleware_1.timingSafeEqual)(u.apiKey, apiKey));
        return user ?? null;
    }
    /**
     * Verify JWT token
    ...
    findUserByUsername(username) {
        try {
            ...
            if (results.length === 0)
                return null;
            return this.stripMeta(JSON.parse(results[0]));
        }
        catch (error) {
            return null;
        }
    ...
    getAllUsers() {
        try {
            ...
            return results.map((r) => this.stripMeta(JSON.parse(r)));
        }
        catch (error) {
            return [];
        }
